  ...
  /** Showing error log  for step that failed */
    ...
    filterScenarios: function (buttonState) {
      ...
      if(buttonState === 'pending'){
        var steps = document.querySelectorAll('.step.pending');
        for (var k = 0; k < steps.length; k++) {
          hasClass = steps[k].classList.contains(buttonState);
          if (hasClass === true) {
            if (steps[k].parentNode.style.display === 'none') {
              steps[k].parentNode.style.display = 'block';
              steps[k].style.display = 'block';
            }
          } else {
            steps[k].parentNode.style.display = 'none';
            steps[k].style.display = 'none';
          }
        }
      }
